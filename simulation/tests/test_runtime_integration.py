...
from simulation.integration.renderer_contract import build_renderer_frame
from simulation.telemetry.stream import build_live_telemetry_frame
...
def test_live_telemetry_frame_exposes_runtime_environment() -> None:
    result = run_simulation(_config())
    frame = build_live_telemetry_frame(result, _config(), launch_datetime_utc="2026-08-19T12:00:00Z")

    assert frame.live is True
    assert frame.temperature_c > -50.0
    assert frame.weather_summary
    assert frame.launch_datetime_utc == "2026-08-19T12:00:00Z"
    assert frame.samples


def test_renderer_frame_preserves_telemetry_for_viewer() -> None:
    result = run_simulation(_config())
    frame = build_renderer_frame(result, _config())

    assert frame["mission_state"]
    assert frame["altitude_m"] >= 0.0
    assert frame["velocity_ms"] >= 0.0
    assert frame["telemetry_points"] >= 1
